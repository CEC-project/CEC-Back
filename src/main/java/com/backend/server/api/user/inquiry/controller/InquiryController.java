...
import com.backend.server.api.user.inquiry.dto.InquiryListRequest;
...
@Tag(name = "4-2. 유저 프로필 / 1:1 문의하기", description = "문의 관련 API")
public class InquiryController {
    ...
    public ApiResponse<Long> createInquiry(
        ...
        return ApiResponse.success("문의글 작성 성공", inquiryId);
    }
    ...
    public ApiResponse<InquiryResponse> getInquiry(
        ...
        return ApiResponse.success("문의글 조회 성공", response);
    }

    @Operation(summary = "내 문의글 목록 조회", description = "로그인한 사용자의 모든 문의글을 페이지네이션과 함께 조회합니다.")
    @GetMapping
    public ApiResponse<InquiryListResponse> getMyInquiries(
            @AuthenticationPrincipal LoginUser loginUser,
            @ParameterObject @ModelAttribute InquiryListRequest request
    ) {
        Long currentUserId = loginUser.getId();
        InquiryListResponse responses = inquiryService.getMyInquiries(currentUserId, request);
        return ApiResponse.success("문의글 목록 조회 성공", responses);
    }

    @Operation(summary = "문의글 수정", description = "본인이 작성한 문의글을 수정합니다.")
    @PutMapping("/{id}")
    public ApiResponse<Long> updateInquiry(
            @PathVariable Long id,
            @Valid @RequestBody InquiryRequest request,
            @AuthenticationPrincipal LoginUser loginUser
    ) throws Exception {
        ...
        return ApiResponse.success("문의글 수정 성공", id);
    }
    ...
    public ApiResponse<Void> deleteInquiry(
        ...
        return ApiResponse.success("문의글 삭제 성공", null);
    }
}
